...
def arlcp_rm(data_source, solution_str, ground_truth, extra_info):
    ...
    # Penalty
    epsilon = 1e-6
    count = count_keywords_counter(solution_str)
    
    # if correct
    if acc == 1:
        ref_score = sigmoid((count - extra_info['information']['mean_refcount']) / (extra_info['information']['std_refcount'] + epsilon))
        len_score = sigmoid((extra_info['information']['solution_len'] - extra_info['information']['mean_corlen']) / (extra_info['information']['std_corlen'] + epsilon))
        ...
        if count <= 40:
            ...
            w1 = 0.15
        w2 = 0.2 - w1
        acc = acc - w1*ref_score - w2*len_score
    ...
    return {
        ...
        "pred": pred,
        "solution_len": extra_info['information']['solution_len'],
        "ref_count": count
    }
